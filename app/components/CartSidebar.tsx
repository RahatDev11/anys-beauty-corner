...
const CartSidebar = () => {
    ...
    const { cart, totalItems, totalPrice, updateQuantity, removeFromCart, checkout } = useCart();
        ...
        <div id="cartSidebar" className={`fixed top-0 right-0 h-full w-80 bg-white shadow-lg transform transition-transform duration-300 ease-in-out z-50 ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
            <div className="p-4 h-full flex flex-col">
                        ...
                        cart.map(item => (
                            <div key={item.id} className="flex items-center justify-between p-2 border-b text-black">
                                <div className="flex items-center">
                                    {item.images && item.images.length > 0 && item.images[0].includes("via.placeholder.com") ? (
                                        <img src={item.images[0]} className="w-10 h-10 object-cover rounded mr-3" alt={item.name} width={40} height={40} />
                                    ) : (
                                        <Image src={item.images && item.images.length > 0 ? item.images[0] : 'https://via.placeholder.com/40'} className="w-10 h-10 object-cover rounded mr-3" alt={item.name} width={40} height={40} />
                                    )}
                                    <div className="flex-grow">
                                        <p className="font-semibold text-sm truncate max-w-[10rem]">{item.name}</p>
                                        <div className="flex items-center">
                                            <button onClick={() => updateQuantity(item.id, -1)} className="px-2 py-1 font-bold text-gray-600 hover:bg-gray-100 rounded-l-lg"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M20 12H4"></path></svg></button>
                                            <span className="px-2 text-sm">{item.quantity}</span>
                                            <button onClick={() => updateQuantity(item.id, 1)} className="px-2 py-1 font-bold text-gray-600 hover:bg-gray-100 rounded-r-lg"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 4v16m8-8H4"></path></svg></button>
                                            <p className="font-semibold text-sm ml-2">{item.price.toFixed(2)}৳</p>
                                        </div>
                                    </div>
                                </div>
                                <button onClick={() => removeFromCart(item.id)} className="text-red-500 hover:text-red-700 ml-auto flex-shrink-0 p-2 rounded-full hover:bg-gray-100"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg></button>
                            </div>
                        ))
                    )}
                </div>
                <div className="cart-footer mt-4">
                    ...
                    <button onClick={checkout} className="w-full bg-lipstick-dark text-white px-4 py-2 rounded mt-2 hover:bg-lipstick transition-colors duration-300">চেকআউট</button>
                </div>
            </div>
        </div>
    );
