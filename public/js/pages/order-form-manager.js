...
function initializeCheckout() {
    ...
    const buyNowData = urlParams.get('buyNowItem'); // Check for the dedicated Buy Now parameter

    if (buyNowData) {
        // If Buy Now data is passed via URL, use it and ignore the regular cart.
        isBuyNowMode = true;
        try {
            checkoutCart = JSON.parse(decodeURIComponent(buyNowData));
            console.log("Buy Now mode activated with items:", checkoutCart);
        } catch (e) {
            console.error("Error parsing Buy Now data:", e);
            showToast("বাই নাও ডেটা লোড করতে সমস্যা হয়েছে।", "error");
            checkoutCart = [];
        }
    } else {
